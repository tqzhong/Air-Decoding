...
class RobertaForPreTraining(RobertaPreTrainedModel):
    ...
    def forward(self, input_ids, attention_mask=None, token_type_ids=None, sentiment=None):
        ...
        for i in range(batch_size):
            senti = pred_sentiment[i].cpu()
            if senti == 0:
                tp0 += list((pred_sentiment.view(-1) == sentiment.view(-1)))[i]
                fp0 += list((pred_sentiment.view(-1) != sentiment.view(-1)))[i]
            elif senti == 1:
                tp1 += list((pred_sentiment.view(-1) == sentiment.view(-1)))[i]
                fp1 += list((pred_sentiment.view(-1) != sentiment.view(-1)))[i]
            else:
                raise Exception("Sentiment Label Error")
        
        loss = loss_fct(seq_relationship_score.view(-1, 2), sentiment.view(-1))
        return loss, tp, tp0, tp1, fp0, fp1
